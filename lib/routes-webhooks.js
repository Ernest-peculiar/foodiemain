...
module.exports = function setupWebhookRoutes(app, dependencies) {
  const seenWebhookMessageIds = new Map();
        ...
        for (const change of entry.changes || []) {
          ...
          for (const message of messages) {
            if (
              !shouldProcessIncomingWebhookMessage(
                message,
                seenWebhookMessageIds,
              )
            ) {
              if (DEBUG)
                console.log(
                  `Skipping duplicate WhatsApp webhook message: ${message?.id || "unknown"}`,
                );
              continue;
            }
  ...
  app.post("/webhook/paystack", async (req, res) => {
    console.log(
      `📩 /webhook/paystack hit — event=${req.body?.event || "(no event field)"} reference=${req.body?.data?.reference || "(none)"}`,
    );
    ...
    if (!PAYSTACK_SECRET_KEY) {
      console.error(
        "🔴 PAYSTACK_SECRET_KEY is not set — rejecting webhook with 400.",
      );
      return res.sendStatus(400);
    }
    if (!signature) {
      console.error(
        "🔴 Request had no x-paystack-signature header — rejecting with 400.",
      );
      return res.sendStatus(400);
    }
    if (!req.rawBody) {
    ...
    if (expectedHash !== signature) {
      console.warn(
        "🔴 Paystack webhook signature mismatch — ignoring request.",
      );
    ...
    if (event?.event === "charge.success") {
      console.log(
        `Processing charge success for reference ${event.data?.reference}`,
      );

...
module.exports = function createStageHandlers(dependencies) {
  ...
  function getCartActionReply(cart) {
    const summary = cart
    ...
    return {
      ...
      interactive: {
        type: "button",
        body: {
          text: `Cart: ${summary}\nChoose another item or finish your selection.`,
        },
        action: {
          buttons: [
            {
              type: "reply",
              reply: { id: "add_another_item", title: "Add another item" },
            },
            {
              type: "reply",
              reply: { id: "done_selecting", title: "Done selecting" },
            },
          ],
        },
      },
  ...
  async function handleGreeting(
    ...
    if (isReturning) {
      if (profile.lastOrder) {
        return {
          replies: [getReorderButtonsReply(profile.lastOrder, name)],
          nextStage: null,
        };
      }
      return {
        replies: [
          {
            type: "text",
            body: `Welcome back, ${name}! 👋 What are you hungry for today?`,
          },
          getGreetingButtonsReply(),
        ],
        nextStage: null,
    ...
    if (phone)
      await saveProfile(phone, { firstSeenAt: new Date().toISOString() });
    ...
    return {
      replies: [
        { type: "text", body: greetingText },
        {
          ...
          body:
            grokReply ||
            `I'm here to help you order food, find nearby restaurants, or get meal ideas.`,
        },
        getNewUserButtonsReply(),
      ],
      nextStage: null,
    ...
    return {
      replies: {
        ...
        interactive: {
          type: "button",
          body: {
            text: `Welcome back, ${shortName}! You were in the middle of ${label}. Pick up where you left off, or start fresh?`,
          },
          action: {
            buttons: [
              {
                type: "reply",
                reply: { id: "resume_flow", title: "▶️ Resume" },
              },
              {
                type: "reply",
                reply: { id: "start_over", title: "🔄 Start over" },
              },
            ],
          },
        },
  ...
  async function getStageResumeReply(session) {
    switch (session.stage) {
      ...
      case STAGES.ASK_MOOD:
        return [
          { type: "text", body: `What are you in the mood for?` },
          getMoodButtonsReply(),
        ];
      ...
      case STAGES.ORDER_SELECT_RESTAURANT:
        return session.registeredVendors?.length
          ? [
              { type: "text", body: `Here are our restaurants again 👇` },
              getRegisteredVendorListReply(session.registeredVendors),
            ]
          : {
              type: "text",
              body: `Which restaurant would you like to order from?`,
            };
      case STAGES.ORDER_SELECT_COMBO:
        return session.menuItems?.length
          ? [
              { type: "text", body: `Here's the menu again 👇` },
              ...[].concat(
                getVendorMenuListReply(
                  session.menuItems,
                  `Menu for ${session.selectedVendor?.name || "this restaurant"}`,
                ),
              ),
              ...(Array.isArray(session.cart) && session.cart.length
                ? [getCartActionReply(session.cart)]
                : []),
            ]
      ...
      case STAGES.ORDER_AWAIT_ADDRESS:
        return [
          ...
          session.previousLocations.length
            ? [
                getPreviousLocationsListReply(
                  session.previousLocations,
                  `Choose a previous delivery spot 👇`,
                ),
              ]
            : []),
        ];
      case STAGES.DRIVER_REG_AWAIT_PHOTO:
        return {
          type: "text",
          body: `📸 Please take a photo of yourself right now using WhatsApp's camera (not from your gallery) to continue registration.`,
        };
      case STAGES.DRIVER_AWAIT_VEHICLE_TYPE:
        return [
          { type: "text", body: `What do you ride for deliveries?` },
          {
  ...
  async function handleOrderAskWhat(text, name, session) {
    ...
    if (vendorName) {
      ...
      if (vendorRecord && hasMenuContent(vendorRecord)) {
      ...
      return handleBrowseRestaurants(
        `I couldn't find *${titleCase(vendorName)}* among our registered restaurants. Here's who is available 👇`,
      );
    }
  ...
  function handleReorderLast(profile) {
    ...
    const vendor = {
      name: lastOrder.vendorName,
      vicinity: lastOrder.vendorVicinity || "Previously used restaurant",
    };

    const previousLocations = [
      lastOrder.address,
      ...(profile.previousLocations || []),
    ]
      ...
      replies: [
        ...
        ...(previousLocations.length
          ? [
              getPreviousLocationsListReply(
                previousLocations,
                `Choose a previous delivery spot for *${vendor.name}* 👇`,
              ),
            ]
          : []),
      ],
  ...
  async function handleAskHealthGoals(text, name, session, shortName) {
    ...
    const recommendations = await buildMoodReply(
      selectedMood,
      shortName,
      session.lastMeal || "something",
    );
    const browseReply = await handleBrowseRestaurants(
      `Here are our registered restaurants — tap one to see if they've got something *${selectedMood}* 👇`,
    );
    ...
    return {
      replies: [
        ...
        ...(Array.isArray(recommendations)
          ? recommendations
          : [recommendations]),
        ...(Array.isArray(browseReply.replies)
          ? browseReply.replies
          : [browseReply.replies]),
      ],
  ...
  async function handleOrderSelectRestaurant(text, name, session) {
    ...
    if (
      trimmed.startsWith("regvendor_") &&
      Array.isArray(session.registeredVendors)
    ) {
      ...
      if (!vendorRecord) {
        return handleBrowseRestaurants(
          `Please tap a restaurant from the list above 👆`,
        );
      }

      return buildVendorMenuReply(
        vendorRecord,
        `Great pick! Here's the menu for *${vendorRecord.name}* 👇`,
      );
    }

    if (trimmed.length < 2) {
      return handleBrowseRestaurants(
        `Please tap a restaurant from the list above 👆, or type the restaurant's name.`,
      );
    }
    ...
    return handleBrowseRestaurants(
      `I couldn't find *${titleCase(trimmed)}* among our registered restaurants. Here's who is available 👇`,
    );
  }
  ...
  async function handleOrderSelectCombo(text, name, session) {
    ...
    if (trimmed === "add_another_item") {
      return {
        replies: [
          { type: "text", body: `Choose another item from the menu 👇` },
          ...[].concat(
            getVendorMenuListReply(
              menuItems,
              `Menu for ${session.selectedVendor?.name || "this restaurant"}`,
          ...
          ...(cart.length ? [getCartActionReply(cart)] : []),
        ],
    ...
    if (trimmed === "done_selecting") {
      if (!cart.length) {
        return {
          replies: {
            type: "text",
            body: `Please choose at least one menu item first 👆`,
          },
          nextStage: STAGES.ORDER_SELECT_COMBO,
          sessionData: {
            selectedVendor: session.selectedVendor,
      ...
      return {
        replies: [
          {
            type: "text",
            body: `Great choice! Select a previous delivery address below, or choose *Type a new address*.`,
          },
          ...(previousLocations.length
            ? [
                getPreviousLocationsListReply(
                  previousLocations,
                  `Choose a delivery address for *${session.selectedVendor?.name || "this restaurant"}* 👇`,
                ),
              ]
            : [
                {
    ...
    if (trimmed.startsWith("proceed_qty_item_")) {
      ...
      if (!combo) {
        return {
          replies: {
            type: "text",
            body: `Something went wrong — please pick a menu item again 👆`,
          },
          nextStage: STAGES.ORDER_SELECT_COMBO,
          sessionData: {
            selectedVendor: session.selectedVendor,
      ...
      return {
        replies: {
          type: "text",
          body: `*${combo.title || combo.name}* is a great choice! How many would you like? (e.g. "2")`,
        },
    ...
    if (trimmed.startsWith("item_")) {
      ...
      if (!combo) {
        return {
          replies: {
            type: "text",
            body: `Please tap a menu item from the list above 👆`,
          },
          nextStage: STAGES.ORDER_SELECT_COMBO,
          sessionData: {
            selectedVendor: session.selectedVendor,
    ...
    return {
      ...
      nextStage: STAGES.ORDER_SELECT_COMBO,
      sessionData: { selectedVendor: session.selectedVendor, menuItems, cart },
    };
  }

  async function handleOrderEnterQty(text, name, session, shortName, phone) {
    const menuItems = Array.isArray(session.menuItems) ? session.menuItems : [];
    ...
    const cartItem = {
      itemIdx: session.selectedComboIdx,
      title: combo.title || combo.name,
      price: Number(combo.price) || 0,
      qty,
    };
    const existing = cart.find((item) => item.itemIdx === cartItem.itemIdx);
    if (existing) existing.qty = Math.min(existing.qty + qty, 20);
    else cart.push(cartItem);

    const profile = phone ? await getProfile(phone) : {};
    const previousLocations = [
      ...(profile.previousLocations || []),
      ...(profile.lastOrder?.address ? [profile.lastOrder.address] : []),
    ]
      .map((loc) => String(loc || "").trim())
      .filter(Boolean)
      .filter((loc, idx, arr) => arr.indexOf(loc) === idx)
    ...
    return {
      replies: [
        {
          type: "text",
          body: `Added *${cartItem.title}* (x${qty}) to your cart.`,
        },
        getCartActionReply(cart),
      ],
      nextStage: STAGES.ORDER_SELECT_COMBO,
      sessionData: {
        selectedVendor: vendor,

  ...
  function parseVendorMenu(menuText, existingItems = []) {
    ...
    return items
      .map((line) => {
        ...
        if (!normalizedLine || /^drinks:?$/i.test(normalizedLine)) return null;

        const match = normalizedLine.match(
          /^(.*?)(?:\s*[-–—:]\s*)(\d[\d,]*(?:\.\d+)?)(?:\s+(?:and|with|plus|white|takeaway|chicken).*?)?$/i,
        );
        ...
        if (!match) {
          ...
          const title = fallbackMatch[1].trim().replace(/\s+[-–—:]\s*$/g, "");
          const parsedPrice = Number(String(fallbackMatch[2]).replace(/,/g, ""));
          const price = Number.isFinite(parsedPrice) ? Math.round(parsedPrice) : null;
        ...
        const title = match[1].trim().replace(/\s+(?:with|white|and|plus)\s+.*$/i, "").trim();

  ...
  function getDriverAcceptButtonsReply(orderId) {
    return {
      ...
      interactive: {
        ...
        action: {
          buttons: [
            { type: 'reply', reply: { id: `driver_reject_${orderId}`, title: '❌ Decline' } },
  ...
  function getDriverPickedUpButtonsReply(orderId, bodyText = 'Let us know when you have the order in hand.') {
    return {
      ...
      interactive: {
        ...
        action: {
          buttons: [
            { type: 'reply', reply: { id: `driver_picked_up_${orderId}`, title: 'Picked up' } }
          ]
        }
      }
    };
  ...
  function getCustomerConfirmDeliveryButtonsReply(orderId, bodyText = 'Has your order arrived?') {
    return {
      ...
      interactive: {
        ...
        action: {
          buttons: [
            { type: 'reply', reply: { id: `customer_complaint_${orderId}`, title: '❌ Problem' } },
            { type: 'reply', reply: { id: `customer_confirmed_${orderId}`, title: '✅ Arrived' } }
          ]
        }
      }
    };

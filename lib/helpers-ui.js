...
module.exports = function createUIHelpers(dependencies) {
  ...
  function parseVendorMenu(menuText, existingItems = []) {
    ...
    for (const line of lines) {
      ...
      if (!normalized || /^drinks?:?$/i.test(normalized)) continue;
    ...
    return validLines
      .map((line) => {
        let match = line.match(
          /^(.*?)[-–—:](\d[\d,]*(?:\.\d+)?)(?:\s*(?:and|with|plus|white|takeaway|chicken).*?)?$/i,
        );
        if (!match) {
          match = line.match(/^(.*?)(\d[\d,]*(?:\.\d+)?)(?:\s+.*)?$/i);
        }
        ...
        if (!title || !priceValue) return null;

        title = title
          .replace(/[-–—:]+\s*$/, "")
          .replace(
            /\s+(?:with|and|plus|white|takeaway|chicken|small|large)\b.*$/i,
            "",
          )
          .trim();
  ...
  function getVendorMenuListReply(items, bodyText = "Menu") {
    ...
    const pages = [];
    const pageCount = Math.ceil(safeItems.length / MAX_ROWS_PER_SECTION);

    for (let i = 0; i < safeItems.length; i += MAX_ROWS_PER_SECTION) {
      const slice = safeItems.slice(i, i + MAX_ROWS_PER_SECTION);
      ...
      pages.push({
        ...
        interactive: {
          ...
          body: {
            text:
              pageCount > 1
                ? `${bodyText} (Menu ${pageNumber}/${pageCount})`
                : bodyText,
          },
          action: {
            button: "Choose",
            sections: [
              {
                title:
                  pageCount > 1 ? `Menu ${pageNumber}/${pageCount}` : "Menu",
                rows: slice.map((item, idx) => ({
                  id: `item_${i + idx}`,
  ...
  function getPreviousLocationsListReply(
    locations,
    bodyText = "Choose a previous delivery location",
  ) {
    const safeLocations = [
      ...new Set(
          ...
          sections: [
            {
              ...
              rows: [
                ...safeLocations.map((location, idx) => ({
                  id: `prev_location_${idx}`,
                  title: location.slice(0, 24),
                  description: "Use a saved address".slice(0, 72),
                })),
                {
                  id: "new_address",
                  title: "Type a new address",
                  description: "Enter a different delivery spot",
                },
              ],
            },
          ],

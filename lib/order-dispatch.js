...
// Ensures a Storage bucket exists AND is public, self-healing on every call
// rather than only on first creation.
//
// FIXED: the old version only passed `public: true` to createBucket(), which
// Supabase only honors the very first time a bucket is created. If the
// bucket already existed — created manually in the dashboard, by an older
// deploy, or by anyone before `public: true` was added to this code — the
// createBucket() call just hits the "already exists" branch and does
// nothing further, silently leaving the bucket PRIVATE. Uploads still
// succeed and getPublicUrl() still happily returns a URL, but that URL
// isn't actually fetchable by WhatsApp's servers, so any image message
// using it fails — and fails *silently*, since sendWhatsAppMessage only logs
// a console.error rather than surfacing anything to the user.
//
// Now, whenever the bucket already exists, we explicitly call
// updateBucket({ public: true }) to force it public — this makes the check
// self-healing on every request instead of a one-time, easy-to-miss setting.
async function ensureBucketIsPublic(supabase, bucketName, options = {}) {
  if (!supabase) return false;
  const { fileSizeLimit = 5242880 } = options;

  try {
    await supabase.storage.createBucket(bucketName, { public: true, fileSizeLimit });
    return true;
  } catch (error) {
  ...
  try {
    ...
    if (updateError) {
      console.error(`Could not force "${bucketName}" bucket public — images from it may fail to send:`, updateError.message);
    }
  } catch (error) {
    console.error(`Could not force "${bucketName}" bucket public — images from it may fail to send:`, error.message || error);
  }
...
async function ensureDeliveryPhotoBucket(supabase) {
  return ensureBucketIsPublic(supabase, 'delivery-photos');
}
...
// Same pattern as ensureDeliveryPhotoBucket/uploadDeliveryPhoto above, but
// for the rider-onboarding selfie into its own bucket. Kept as a separate
// bucket (rather than reusing delivery-photos) so rider ID photos and
// delivery-proof photos have distinct storage/retention/access rules if you
// ever need them — e.g. only admins should be able to browse rider selfies,
// whereas delivery photos might reasonably be visible to the customer who
// placed that order.
async function ensureDriverPhotoBucket(supabase) {
  return ensureBucketIsPublic(supabase, 'driver-photos');
}

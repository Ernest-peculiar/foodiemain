...
//
// UPDATED: now also accepts/persists `menuItems` (-> menu_items), the
// structured per-item menu (id/title/price/available) used for vendor menu
// management ("menu" / "edit menu" commands, and the toggle-item-available
// flow). `menu` (the raw text) is kept as-is for backward compatibility and
// as a human-readable record — `menu_items` is the source of truth for
// availability. Like the other fields here, `menuItems` only overwrites the
// existing value when this call actually supplied one (via `??`), so a
// caller that only touches, say, `isOpen` never accidentally wipes out a
// vendor's structured menu.
//
// REQUIRES a new nullable column on the `vendors` table — run this once in
// Supabase if it doesn't already exist:
//
...
// NEW — cheap, targeted update for the "toggle one menu item's
// available/sold-out status" flow (tapping an item in the list shown by the
// "menu" command). Writes only `menu_items`, rather than routing a single
// tap through the full upsertVendor(), which expects a `name` and would
// otherwise require re-fetching + re-passing every other vendor field just
// to flip one boolean.
//
// `menuItems` should be the full, already-modified array (i.e. the caller
// flips the one item's `available` flag locally, then passes the whole
// array here to persist it) — this does a plain replace, not a merge.

...
async function resolveOrCreateVendor(supabase, vendor) {
  ...
  if (phone) {
    ...
    if (error) {
      console.error(
        "Vendor lookup by phone failed (resolveOrCreateVendor):",
        error.message,
      );
    } else {
      existing = data;
  ...
  if (!existing) {
    ...
    if (error) {
      console.error(
        "Vendor lookup by name failed (resolveOrCreateVendor):",
        error.message,
      );
    } else {
      existing = data;
...
async function upsertDriver(supabase, driver) {
  ...
  if (phone) {
    const { data, error: lookupError } = await supabase
      .from("drivers")
      .select(
        "id, name, phone, is_active, is_online, current_order, photo_url, vehicle_type",
      )
      .eq("phone", phone)
      .maybeSingle();
...
async function createOrderRecord(supabase, payload) {
  ...
  if (vendor && vendor.is_active === false) {
    console.warn(
      `createOrderRecord: vendor ${vendor.id} (${vendor.name}) is inactive — refusing to create order.`,
    );
    return null;
  }
  if (vendor && vendor.is_open === false) {
    console.warn(
      `createOrderRecord: vendor ${vendor.id} (${vendor.name}) is closed — refusing to create order.`,
    );
    return null;
  }
  ...
  if (!orderPayload.customer_phone) {
    console.error(
      `createOrderRecord: no customer_phone in payload for order to ${orderPayload.restaurant_name} — receipts/notifications for this order will silently have nowhere to go.`,
    );
  }
...
async function assignDriverToOrder(supabase, orderId, driverId) {
  ...
  if (typeof supabase.rpc === "function") {
    try {
      ...
      console.warn(
        "assign_driver_to_order RPC failed, falling back to optimistic update:",
        error.message,
      );
    } catch (error) {
      console.warn(
        "assign_driver_to_order RPC threw, falling back to optimistic update:",
        error.message,
      );
    }
  }
...
async function setVendorAvailability(supabase, phone, isOpen) {
  ...
  const existing = await supabase
    ...
    .maybeSingle();
  const record = existing?.data || existing;
  return upsertVendor(supabase, {
    phone,
    name: record?.name || "Vendor",
    isActive: true,
    isOpen,
  });
}
...
async function ensureDeliveryPhotoBucket(supabase) {
  ...
  try {
    await supabase.storage.createBucket("delivery-photos", {
      public: true,
...
async function uploadDeliveryPhoto(
  ...
  const { data: publicData } = supabase.storage
    .from("delivery-photos")
    .getPublicUrl(data?.path || objectName);

...
module.exports = function createDispatchHandlers(dependencies) {
  ...
  async function handleVendorMenuCommands(text, phone, session, supabase) {
    ...
    const items = normalizeMenuItems(vendor.menu_items);
    const menuItems = items.length > 0 ? items : parseVendorMenu(vendor.menu);
  ...
  async function finalizeRegistration(text, phone, session, supabase) {
    ...
    if (role === "vendor") {
      ...
      if (vendor?.phone) {
        const welcomeMessage = buildVendorWelcomeMessage(vendorName, menuItems);
        await sendWhatsAppMessage(normalizePhone(vendor.phone), welcomeMessage);
      }
  ...
  async function handleDispatchPayload(payload, phone, session, supabase) {
    ...
    if (toggleItemMatch) {
      ...
      try {
        await dispatch.updateVendorMenuItems(supabase, vendor.id, menuItems);
      } catch (error) {
        console.error(

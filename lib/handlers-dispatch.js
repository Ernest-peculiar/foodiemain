...
module.exports = function createDispatchHandlers(dependencies) {
  const { STAGES, dispatch, normalizePhone, parseVendorMenu, sendWhatsAppMessage, getVendorRecordByPhone, getDriverRecordByPhone, getVendorMenuManagementListReply, getDriverAcceptButtonsReply, getDriverPickedUpButtonsReply, getCustomerConfirmDeliveryButtonsReply, getVehicleTypeListReply, WHATSAPP_TOKEN, WHATSAPP_API_VERSION, ORDER_NOTIFY_NUMBER } = dependencies;
  ...
  async function handleDispatchPayload(payload, phone, session, supabase) {
        ...
        if (orderRow.vendor_id) {
          ...
          if (vendorTarget) {
            if (driver.photo_url) {
              ...
              await sendWhatsAppMessage(vendorTarget, { type: 'text', body: `✅ ${driver.name} has accepted the delivery and is on the way to pick up the order.` });
            }
          }
        }
      }
    ...
    const confirmDeliveryMatch = normalized.match(/^customer_confirmed_([0-9a-f-]+)$/i);
    ...
    const reportIssueMatch = normalized.match(/^customer_complaint_([0-9a-f-]+)$/i);
    if (reportIssueMatch) {

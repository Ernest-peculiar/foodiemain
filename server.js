...
const { STAGES, STAGE_LABELS, MOOD_KEYWORDS, MOOD_CATALOG } = require('./lib/constants');
const {
  normalizePhone,
  parseEmail,
  parseOrderRequest,
...
const { getSession, setSession, deleteSession, getProfile, saveProfile, resolveSenderRole, logMessage, getVendorRecordByPhone, getVendorRecordById, getDriverRecordByPhone, findVendorByName, getRegisteredVendors, mapMoodToCategory } = db;

const createUIHelpers = require('./lib/helpers-ui');
const uiHelpers = createUIHelpers({ MOOD_CATALOG, PUBLIC_URL });
const { getNewUserButtonsReply, getGreetingButtonsReply, getReorderButtonsReply, getHungryButtonsReply, getMoodButtonsReply, getPostVendorButtonsReply, getVendorPrepTimeButtonsReply, getDriverDeliveryButtonsReply, getDriverAcceptButtonsReply, getDriverPickedUpButtonsReply, getCustomerConfirmDeliveryButtonsReply, getRegisteredVendorListReply, getVendorMenuListReply, parseVendorMenu, makeItemId } = uiHelpers;

const createStageHandlers = require('./lib/handlers-stages');
const stageHandlers = createStageHandlers({
  STAGES, STAGE_LABELS, MOOD_KEYWORDS, MOOD_CATALOG,
  getRegisteredVendors, findVendorByName, titleCase, parseOrderRequest, isLikelyValidAddress, mapMoodToCategory,
  buildMoodReply, buildVendorMenuReply, handleBrowseRestaurants,
  saveProfile, askGrok, parseVendorMenu, makeItemId, createPaystackTransaction, DELIVERY_FEE,
  getMoodButtonsReply, getVendorMenuListReply, getHungryButtonsReply, getNewUserButtonsReply, getGreetingButtonsReply, getReorderButtonsReply, getRegisteredVendorListReply
});
const { handleGreeting, handleResumePrompt, getStageResumeReply, handleCapabilities, handleHungry, handleRecommendMeals, handleOrderAskWhat, handleOrderNow, handleReorderLast, handleAskLastMeal, handleAskMood, handleAskHealthGoals, handleOrderSelectRestaurant, handleOrderSelectCombo, handleOrderEnterQty, handleMealPlanPlaceholder } = stageHandlers;

const createPaymentHandlers = require('./lib/payment');
const paymentHandlers = createPaymentHandlers({
  DELIVERY_FEE, WHATSAPP_TOKEN, WHATSAPP_PHONE_NUMBER_ID, WHATSAPP_API_VERSION, PAYSTACK_SECRET_KEY, PUBLIC_URL,
  sendWhatsAppMessage, saveProfile, getVendorRecordById
});
const { createPaystackTransaction, generateReceiptPNG, uploadWhatsAppMedia, buildPaymentSuccessPage, buildReceiptText, handlePaystackChargeSuccess } = paymentHandlers;

const createDispatchHandlers = require('./lib/handlers-dispatch');
const dispatchHandlers = createDispatchHandlers({
  STAGES, dispatch, normalizePhone, parseVendorMenu, sendWhatsAppMessage,
  getVendorRecordByPhone, getDriverRecordByPhone, getVendorMenuManagementListReply: uiHelpers.getVendorMenuManagementListReply,
  getDriverAcceptButtonsReply, getDriverPickedUpButtonsReply, getCustomerConfirmDeliveryButtonsReply,
  getVehicleTypeListReply: () => ({}),  // placeholder
  WHATSAPP_TOKEN, WHATSAPP_API_VERSION, ORDER_NOTIFY_NUMBER
});
const { handleAvailabilityCommands, handleVendorMenuCommands, handleRegistrationFlow, finalizeRegistration, handleDispatchPayload, handleDeliveryPhotoMessage } = dispatchHandlers;
...
async function sendWhatsAppMessage(toPhone, message) {
  ...
  const payload = { messaging_product: 'whatsapp', recipient_type: 'individual', to: normalizedPhone };
  ...
  if (message.type === 'text') {
    ...
    payload.type = 'image';
    payload.image = message.mediaId ? { id: message.mediaId } : { link: message.imageUrl };
  ...
  try {
    ...
    if (!response.ok) {
      ...
      console.error(`WhatsApp API error (${response.status}):`, error.error?.message || JSON.stringify(error));
      return null;
    }
...
async function handleBrowseRestaurants(introText = `Here are our registered restaurants 👇 Tap one to see the menu.`) {
  ...
  if (vendors.length === 0) {
    return {
      replies: { type: 'text', body: `No registered restaurants are available right now. Please check back later.` },
      nextStage: null,
      sessionData: {}
    };
  }
...
function buildVendorMenuReply(vendorRecord, introText) {
  ...
  if (menuItems.length === 0) {
    return {
      replies: { type: 'text', body: `Sorry, *${vendor.name}* has nothing available right now — please check back later.` },
      nextStage: null,
      sessionData: {}
    };
  }
...
async function handleOrderAwaitAddress(text, name, session, shortName, phone) {
  ...
  if (!isLikelyValidAddress(text)) {
    return {
      ...
      sessionData: { selectedVendor: vendor, selectedComboIdx: session.selectedComboIdx, qty, menuItems }
    };
  }
  ...
  try {
    orderRecord = await dispatch.createOrderRecord(supabase, {
      customerName: name,
      customerPhone: phone,
      vendor: { id: vendor.id || null, name: vendor.name, phone: vendor.phone || null, isActive: true, isOpen: true },
  ...
  if (!orderRecord?.id) {
    console.error('createOrderRecord did not return an order id — order was not created.');
    return {
...
async function buildReply(text, name = 'friend', session = {}, phone) {
  ...
  if (normalized === 'resume_flow') return { replies: await getStageResumeReply(session), nextStage: session.stage, sessionData: session };
  ...
  return {
    replies: { type: 'text', body: `Just say you're hungry and I'll guide you through finding the perfect meal! 😊` },
    nextStage: null
  };
}
...
async function handleIncomingMessage(message, value) {
  ...
  const menuManagementReply = registrationReply ? null : await handleVendorMenuCommands(text, from, session, supabase);
  ...
  if (registrationReply) {
    ...
    result = {
      replies: { type: 'text', body: `No need to share your location — just tell me what you'd like and which restaurant it's from, e.g. "rice from Munchy".` },
      nextStage: null
    };
  } else {
    ...
    const availabilityReply = await handleAvailabilityCommands(text, from, session, supabase);
    if (availabilityReply) {
      result = availabilityReply;
    } else if (role === 'vendor') {
      const dispatchReply = await handleDispatchPayload(text, from, session, supabase);
...
setupAdminRoutes(app, {
  supabase, dispatch, normalizePhone, crypto, path, fs, express,
  ADMIN_USERNAME, ADMIN_PASSWORD, PAYSTACK_SECRET_KEY, PUBLIC_URL, WHATSAPP_TOKEN, sendWhatsAppMessage
});

// Setup webhook routes
const setupWebhookRoutes = require('./lib/routes-webhooks');
setupWebhookRoutes(app, {
  VERIFY_TOKEN, PAYSTACK_SECRET_KEY, crypto, isValidWhatsAppPayload, handleIncomingMessage, handlePaystackChargeSuccess, buildPaymentSuccessPage, supabase, ORDER_NOTIFY_NUMBER
});
...
app.get('/paystack/callback', async (req, res) => {
  ...
  try {
    ...
    if (result?.status && result?.data?.status === 'success') {
      try {
        const paymentDeps = { supabase, ORDER_NOTIFY_NUMBER, getVendorPrepTimeButtonsReply };
        await handlePaystackChargeSuccess(result.data, supabase, ORDER_NOTIFY_NUMBER, getVendorPrepTimeButtonsReply);
      } catch (error) {
...
╰────────────────────────────────────╯
`);
  if (!supabase) console.warn('⚠️  Running without Supabase — chat history and persistence disabled.');
  if (!GROK_API_KEY) console.warn('⚠️  Running without Grok API — AI-powered replies disabled.');
  if (!PAYSTACK_SECRET_KEY) console.warn('⚠️  Running without Paystack — payment processing disabled.');
});
